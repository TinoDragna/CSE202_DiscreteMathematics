...
class B19_WTRABS_Overflow {
    // b1. check m = n-1
    // b2. check connected
    // chậu trên chia đều lượng nước nó có cho chậu con của nó
    ...
    public static void main(String[] args) throws IOException {
        ...
        for (Vertex v : graph) {
            if (v.nOfWater != 0) {
                sb.append(v.id + " " + v.nOfWater).append("\n");
            }
        }
    ...
    static void dfs(Vertex v) {
        ...
        if (v.adjList.size() > 0) {
            for (Vertex w : v.adjList) {
                if (!w.discovered) {
                    w.nOfWater += v.nOfWater / v.adjList.size();
                    dfs(w);
                }
            }
            v.nOfWater = 0;
    ...
    static Vertex[] readGraph(int nOfVertex, int nOfEdges) {
        ...
        for (int i = 0; i < vertices.length; i++) {
            double nOfWater = sc.nextDouble();
            vertices[i] = new Vertex(i, nOfWater);
        }
    ...
    static class Vertex {
        ...
        public Vertex(int id, double nOfWater) {
            this.id = id;
            this.nOfWater = nOfWater;
        }
