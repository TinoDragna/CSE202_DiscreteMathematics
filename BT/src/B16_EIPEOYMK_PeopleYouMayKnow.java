...
class B16_EIPEOYMK_PeopleYouMayKnow {
    ...
    public static void main(String[] args) throws IOException {
        ...
        for (Vertex v : graph) {
            if (v.level != -1) {
                if (!levelList.containsKey(v.level)) {
                    levelList.put(v.level, new ArrayList<>());
                }
                levelList.get(v.level).add(v.id);
            }
        }

        for (int i = 0; i < q; i++) {
            int k = sc.nextInt();
            List<Integer> friends = levelList.get(k);
